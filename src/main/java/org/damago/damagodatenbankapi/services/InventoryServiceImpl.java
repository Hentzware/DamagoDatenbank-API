...
public class InventoryServiceImpl implements InventoryService {
    ...
    public InventoryResponse Add(AddInventoryRequest request) {
        String id = inventarRepository.sp_Inventories_Add(request.getName(), request.getAmount());
        Inventory inventory = inventarRepository.sp_Inventories_GetById(id);
    ...
    public void Delete(DeleteInventoryRequest request, boolean permanent) {
        if (permanent) {
            inventarRepository.sp_Inventories_DeletePermanent(request.getId());
            return;
        }
        inventarRepository.sp_Inventories_Delete(request.getId());
    }

    @Override
    public InventoryResponse Edit(EditInventoryRequest request) {
        inventarRepository.sp_Inventories_Update(request.getId(), request.getName(), request.getAmount());
        Inventory inventory = inventarRepository.sp_Inventories_GetById(request.getId());
        return modelMapper.map(inventory, InventoryResponse.class);
    }
    ...
    public Iterable<InventoryResponse> Get(boolean deleted) {
        ...
        if (deleted) {
            inventories = inventarRepository.sp_Inventories_GetDeleted();
        } else {
            inventories = inventarRepository.sp_Inventories_Get();
        }
    ...
    public Iterable<InventoryResponse> Search(SearchInventoryRequest request) {
        Iterable<Inventory> inventories = inventarRepository.sp_Inventories_Search(request.getName(), request.getAmount());
        return modelMapper.map(inventories, new TypeToken<Iterable<InventoryResponse>>() {
        }.getType());
    }
}
